...
| **`DocumentScrollNavigationEngine`** | Continuous active-page detection from viewport geometry, shared page-rest anchor (top) for programmatic navigation, and suppression so free scrolling never auto-jumps. |
...
Floating page chrome (`pageBottomToolbar` / `floatingPageToolbar` / circular `pageModeAddButton`) uses ultra-thin material consistent with the top bar, fades while scrolling, and returns after scroll settles. Pages share equal scale with a soft active-page halo (`documentPageSheetChrome`). Vertical scrolling is free with normal inertia; there is no settle-snap. Active page tracks viewport geometry continuously without moving the document. Document pinch zoom (`DocumentZoomState`, 1×–4×) scales every page frame and gap together; page-local canvas zoom is disabled on this surface. Thumbnails (`pageThumbnail_N` / `documentPageGrid`) appear only after **⋯ → Pages**. Tests that need the grid call `openPagesOrganizer()`. Page editing runs on the active canvas; organizer / search / tap activate pages with explicit scroll. There is no Document Mode → Page Mode push.
