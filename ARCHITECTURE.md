...
| **`DocumentScrollNavigationEngine`** | Continuous active-page detection from viewport geometry, intentional tap-to-activate snap gating (idle only), shared page-rest anchor (top) for explicit navigation, and suppression so free scrolling never auto-jumps. |
| **`DocumentZoomState` / `DocumentZoomEngine`** | Document-owned magnification (1×–4×): scaled page frames/spacing, focal-point / active-page scroll anchoring (never resets to page 1). Page-local pinch is disabled on the unified surface. |
...
Floating page chrome (`pageBottomToolbar` / `floatingPageToolbar` / circular `pageModeAddButton`) uses ultra-thin material consistent with the top bar, fades while scrolling, and returns after scroll settles. Pages share equal scale with a soft active-page halo (`documentPageSheetChrome`). Vertical scrolling is free with normal inertia; there is no post-scroll settle-snap. Intentional tap activation (page / overlay / annotation while idle) smoothly scrolls to `pageRestAnchor` before editing controls apply. Active-page tracking from viewport geometry updates indicators without moving the document. Document pinch zoom (`DocumentZoomState`, 1×–4×) scales every page frame and gap together; page-local canvas zoom is disabled on this surface. Thumbnails (`pageThumbnail_N` / `documentPageGrid`) appear only after **⋯ → Pages**. Tests that need the grid call `openPagesOrganizer()`. Page editing runs on the active canvas; organizer / search / intentional tap activate pages with explicit scroll. There is no Document Mode → Page Mode push.
